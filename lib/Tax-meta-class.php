...
if ( ! class_exists( 'Tax_Meta_Class' ) ) :
	...
	class Tax_Meta_Class {
		...
		public function __construct( $meta_box ) {
			...
			add_action( 'admin_print_styles', [ $this, 'load_scripts_styles' ] );
			
			//overwrite insert into post button
			
			//delete term meta on term deletion
			add_action( 'delete_term', [ $this, 'delete_taxonomy_metadata' ], 10, 2 );
		}
		...
		public function load_scripts_styles() {
			...
			if ( in_array( $taxnow, $this->_meta_box[ 'pages' ] ) ) {
				...
				wp_enqueue_script( 'tax-meta-clss', $plugin_path . '/js/tax-meta-clss.js', [ 'jquery' ], null, true );
		...
		public function add() {
			...
			foreach ( $this->_meta_box[ 'pages' ] as $page ) {
				...
				add_action( $page . '_edit_form_fields', [ $this, 'show_edit_form' ] );
				//add fields to add new form
				add_action( $page . '_add_form_fields', [ $this, 'show_new_form' ] );
				// this saves the edit fields
				add_action( 'edited_' . $page, [ $this, 'save' ], 10, 2 );
				// this saves the add fields
				add_action( 'created_' . $page, [ $this, 'save' ], 10, 2 );
			}
			// Delete all attachments when delete custom post type.
			add_action( 'wp_ajax_at_delete_file', [ $this, 'delete_file' ] );
			add_action( 'wp_ajax_at_reorder_images', [ $this, 'reorder_images' ] );
			// Delete file via Ajax
			add_action( 'wp_ajax_at_delete_mupload', [ $this, 'wp_ajax_delete_image' ] );
		...
		public function show( $term_id ) {
			...
			foreach ( $this->_fields as $field ) {
				...
				call_user_func( [ $this, 'show_field_' . $field[ 'type' ] ], $field,
			...
			if ( count( $meta ) > 0 && is_array( $meta ) ) {
				foreach ( $meta as $me ) {
					...
					foreach ( $field[ 'fields' ] as $f ) {
						...
						if ( $f[ 'type' ] == 'wysiwyg' ) {
							call_user_func( [ $this, 'show_field_' . $f[ 'type' ] ], $f,
											is_array( $m ) ? $m : stripslashes( $m ), true );
						} else {
							call_user_func( [ $this, 'show_field_' . $f[ 'type' ] ], $f,
		...
		<img src="';
					...
					echo '" alt="' . __( 'Remove', 'tax-meta' ) . '" title="' . __( 'Remove',
																					'tax-meta' ) . '" id="remove-' . $field[ 'id' ] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . __( 'Add', 'tax-meta' ) . '" title="' . __( 'Add',
																		 'tax-meta' ) . '" id="add-' . $field[ 'id' ] . '"><br/></div>';
			...
			foreach ( $field[ 'fields' ] as $f ) {
				...
				if ( $f[ 'type' ] == 'wysiwyg' ) {
					call_user_func( [ $this, 'show_field_' . $f[ 'type' ] ], $f, '', true );
				} else {
					call_user_func( [ $this, 'show_field_' . $f[ 'type' ] ], $f, '' );
				}
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_select( $field, $meta ) {
			...
			$this->show_field_begin( $field, $meta );
			echo "<select class='at-select' style='{$field['style']}' name='{$field['id']}" . ( $field[ 'multiple' ]
		...
		public function show_field_radio( $field, $meta ) {
			...
			$this->show_field_begin( $field, $meta );
			foreach ( $field[ 'options' ] as $key => $value ) {
				echo "<input style='{$field['style']}' type='radio' class='at-radio' name='{$field['id']}' value='{$key}'" . checked( in_array( $key,
																																				$meta ),
																																	  true,
																																	  false ) . " /> <span class='at-radio-label'>{$value}</span>";
			}
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_checkbox( $field, $meta ) {
			...
			$this->show_field_begin( $field, $meta );
			echo "<input type='checkbox' style='{$field['style']}' class='rw-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked( ! empty( $meta ),
																																			 true,
																																			 false ) . " />";
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_wysiwyg( $field, $meta, $in_repeater = false ) {
			...
			if ( version_compare( $wp_version, '3.2.1' ) < 1 || $in_repeater ) {
				...
				wp_editor( stripslashes( html_entity_decode( $meta ) ), $field[ 'id' ],
		...
		public function show_field_checkbox_list( $field, $meta ) {
			...
			foreach ( $field[ 'options' ] as $key => $value ) {
				$html[] = "<input style='{$field['style']}' type='checkbox' class='at-checkbox_list' name='{$field['id']}[]' value='{$key}'" . checked( in_array( $key,
																																								  $meta ),
																																						true,
																																						false ) . " /> {$value}";
			}
		...
		public function show_field_posts( $field, $meta ) {
			...
			if ( 'checkbox_list' == $options[ 'type' ] ) {
				foreach ( $posts as $p ) {
					echo "<input type='checkbox' name='{$field['id']}[]' value='$p->ID'" . checked( in_array( $p->ID,
																											  $meta ),
																									true,
																									false ) . " /> $p->post_title<br/>";
				}
			} // select
			else {
				echo "<select name='{$field['id']}" . ( $field[ 'multiple' ]
						? "[]' multiple='multiple' style='height:auto'" : "'" ) . ">";
				foreach ( $posts as $p ) {
					echo "<option value='$p->ID'" . selected( in_array( $p->ID, $meta ), true,
															  false ) . ">$p->post_title</option>";
				}
				echo "</select>";
			}
		...
		public function show_field_taxonomy( $field, $meta ) {
			...
			if ( 'checkbox_list' == $options[ 'type' ] ) {
				foreach ( $terms as $term ) {
					echo "<input type='checkbox' name='{$field['id']}[]' value='$term->slug'" . checked( in_array( $term->slug,
																												   $meta ),
																										 true,
																										 false ) . " /> $term->name<br/>";
				}
				...
				echo "<select name='{$field['id']}" . ( $field[ 'multiple' ]
						? "[]' multiple='multiple' style='height:auto'" : "'" ) . ">";
				foreach ( $terms as $term ) {
					echo "<option value='$term->slug'" . selected( in_array( $term->slug, $meta ), true,
																   false ) . ">$term->name</option>";
				}
				echo "</select>";
			}
		...
		public function save( $term_id ) {
			...
			if ( ! isset( $term_id )                            // Check Revision
				 ...
				 || ( ! in_array( $_POST[ 'taxonomy' ],
								  $this->_meta_box[ 'pages' ] ) )              // Check if current taxonomy type is supported.
				 || ( ! check_admin_referer( basename( __FILE__ ),
											 'tax_meta_class_nonce' ) )    // Check nonce - Security
			...
			foreach ( $this->_fields as $field ) {
				...
				$new = ( isset( $_POST[ $name ] ) ) ? $_POST[ $name ] : ( ( $multiple ) ? [] : '' );
				
				// Validate meta value
				if ( class_exists( 'Tax_Meta_Validate' )
					 && method_exists( 'Tax_Meta_Validate', $field[ 'validate_func' ] ) ) {
					$new = call_user_func( [ 'Tax_Meta_Validate', $field[ 'validate_func' ] ], $new );
				}
				...
				if ( $type != "paragraph" ) {
					...
					if ( method_exists( $this, $save_func ) ) {
						call_user_func( [ $this, 'save_field_' . $type ], $term_id, $field, $old, $new );
					} else {
   ...
   */
					...
					foreach ( $field[ 'fields' ] as $f ) {
						...
						switch ( $type ) {
							case 'wysiwyg':
								...
								break;
							case 'file':
								$n[ $f[ 'id' ] ] = $this->save_field_file_repeater( $term_id, $f, '',
																					$n[ $f[ 'id' ] ] );
								break;
							default:
								break;
		...
		public function add_missed_values() {
			...
			$this->_meta_box = array_merge( [ 'context' => 'normal', 'priority' => 'high', 'pages' => [ 'post' ] ],
		...
		public function enqueue_jqueryui() {
			...
			wp_enqueue_style( 'tmc-jquery-ui-css',
							  'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/' . apply_filters( 'tmc_jquery_ui_theme',
																																		 'smoothness' ) . '/jquery-ui.css' );
			wp_enqueue_script( 'tmc-jquery-ui',
							   'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js',
							   [ 'jquery' ] );
		}
		...
		public function addField( $id, $args ) {
			...
			$new_field       = [ 'id' => $id, 'std' => '', 'desc' => '', 'style' => '', 'multiple' => false ];
		...
		public function addFile( $id, $args, $repeater = false ) {
			...
			$new_field = [ 'type' => 'file', 'id' => $id, 'desc' => '', 'name' => 'File Field', 'multiple' => false ];
			$new_field = array_merge( $new_field, $args );
			if ( false === $repeater ) {
				$this->_fields[] = $new_field;
			} else {
		...
		public function addTaxonomy( $id, $options, $args, $repeater = false ) {
			...
			$tax       = 'category';
			$type      = 'select';
			$temp      = [ 'taxonomy' => $tax, 'type' => $type, 'args' => $q ];
		...
		public function addPosts( $id, $options, $args, $repeater = false ) {
			...
			$temp      = [ 'type' => 'select', 'args' => [ 'posts_per_page' => - 1, 'post_type' => 'post' ] ];
			$options   = array_merge( $temp, $options );
			$new_field = [
				'type'     => 'posts',
				'id'       => $id,
...
if ( ! function_exists( 'get_tax_meta_all' ) ) {
	function get_tax_meta_all( $term_id ) {
		...
		return get_option( 'tax_meta_' . $t_id, [] );
	}
}