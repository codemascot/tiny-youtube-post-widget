	...
	class Tax_Meta_Class {
		...
		public function load_scripts_styles() {
			...
			if ( in_array( $taxnow, $this->_meta_box[ 'pages' ] ) ) {
				...
				wp_enqueue_style( 'tax-meta-clss', $plugin_path . '/lib/css/Tax-meta-class.css' );
				// Enqueue Meta Box Scripts
				wp_enqueue_script( 'tax-meta-clss', $plugin_path . '/lib/js/tax-meta-clss.js', array(
					 'jquery' 
				), null, true );
			} //in_array( $taxnow, $this->_meta_box[ 'pages' ] )
		}
		/**
		...
		public function check_field_color() {
			...
			} //$this->has_field( 'color' ) && $this->is_edit_page()
		}
		/**
		 * Check Field Date
		 *
		...
		public function check_field_date() {
			...
			} //$this->has_field( 'date' ) && $this->is_edit_page()
		}
		/**
		 * Check Field Time
		 *
		...
		public function check_field_time() {
			if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
				// Enqueu JQuery UI, use proper version.
				$this->enqueue_jqueryui();
				wp_enqueue_script( 'at-timepicker', '//cdnjs.cloudflare.com/ajax/libs/jquery-ui-timepicker-addon/1.4.5/jquery-ui-timepicker-addon.min.js', array(
					 'tmc-jquery-ui' 
				), false, true );
			} //$this->has_field( 'time' ) && $this->is_edit_page()
		}
		/**
		...
		public function add() {
			...
			foreach ( $this->_meta_box[ 'pages' ] as $page ) {
				...
				add_action( $page . '_edit_form_fields', array(
					 $this,
					'show_edit_form' 
				) );
				//add fields to add new form
				add_action( $page . '_add_form_fields', array(
					 $this,
					'show_new_form' 
				) );
				// this saves the edit fields
				add_action( 'edited_' . $page, array(
					 $this,
					'save' 
				), 10, 2 );
				// this saves the add fields
				add_action( 'created_' . $page, array(
					 $this,
					'save' 
				), 10, 2 );
			} //$this->_meta_box[ 'pages' ] as $page
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_wysiwyg( $field, $meta, $in_repeater = false ) {
			...
			} //version_compare( $wp_version, '3.2.1' ) < 1 || $in_repeater
			else {
				// Use new wp_editor() since WP 3.3
				wp_editor( stripslashes( html_entity_decode( $meta ) ), $field[ 'id' ], array(
		...
		public function save( $term_id ) {
			...
			if ( isset( $_REQUEST[ 'action' ] ) && $_REQUEST[ 'action' ] == 'inline-save-tax' ) {
				return $term_id;
			} //isset( $_REQUEST[ 'action' ] ) && $_REQUEST[ 'action' ] == 'inline-save-tax'
			if ( !isset( $term_id ) // Check Revision
				...
				{
				return $term_id;
			} //!isset( $term_id ) || ( !isset( $_POST[ 'taxonomy' ] ) ) || ( !in_array( $_POST[ 'taxonomy' ], $this->_meta_box[ 'pages' ] ) ) || ( !check_admin_referer( basename( __FILE__ ), 'tax_meta_class_nonce' ) ) || ( !current_user_can( 'manage_categories' ) )
			foreach ( $this->_fields as $field ) {
				$name     = $field[ 'id' ];
				$type     = $field[ 'type' ];
				$multiple = isset( $field[ 'multiple' ] ) ? $field[ 'multiple' ] : false;
				...
				} //class_exists( 'Tax_Meta_Validate' ) && method_exists( 'Tax_Meta_Validate', $field[ 'validate_func' ] )
				...
				if ( $type != "paragraph" ) {
					...
					if ( method_exists( $this, $save_func ) ) {
						call_user_func( array(
							 $this,
							'save_field_' . $type 
						), $term_id, $field, $old, $new );
					} //method_exists( $this, $save_func )
					else {
		...
		public function save_field_repeater( $term_id, $field, $old, $new ) {
			if ( is_array( $new ) && count( $new ) > 0 ) {
				...
				} //isset( $temp ) && count( $temp ) > 0 && !$this->is_array_empty( $temp )
				else {
				...
				elseif ( 'repeater' == $field[ 'type' ] ) {
					foreach ( (array) $field[ "fields" ] as $repeater_field ) {
						if ( $type == $repeater_field[ "type" ] )
							return true;
					} //(array) $field[ "fields" ] as $repeater_field
				} //'repeater' == $field[ 'type' ]
			} //$this->_fields as $field
			return false;
		}
		...
		public function get_jqueryui_ver() {
			...
			if ( version_compare( $wp_version, '3.1', '>=' ) ) {
				return '1.8.10';
			} //version_compare( $wp_version, '3.1', '>=' )
			return '1.7.3';
		}
		/**
		...
		public function enqueue_jqueryui() {
			...
			wp_enqueue_script( 'tmc-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array(
				 'jquery' 
			) );
		}
		/**
...
} //!function_exists( 'delete_tax_meta' )
//update meta
...
} //!function_exists( 'update_tax_meta' )
...
} //!function_exists( 'get_tax_meta_strip' )
...
} //!function_exists( 'get_tax_meta_all' )